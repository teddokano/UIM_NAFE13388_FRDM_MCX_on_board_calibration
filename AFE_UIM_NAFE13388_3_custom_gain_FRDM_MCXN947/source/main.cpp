...
void	recalibrate( int pga_gain_index, int ch_GND = 14, int ch_REF = 15 );
...
void recalibrate( int pga_gain_index, int ch_GND, int ch_REF )
{
	...
	const uint16_t	REF_H	= REF_GND | (reference_source_selection << 12);
	...
	raw_t	data_REFH	= afe.read<raw_t>( ch_REF, 1.1 );
	...
	const double	calibrated_gain		= (pow( 2, 23 ) - 1.00) * (reference_source_voltage / fullscale_voltage) / (double)(data_REFH - data_GND);
